...
namespace Stribog
{
    class Stribog
    {
        ...
        private byte[] HashAdd512(byte[] a, byte[] b)
        {
            ...
            for (i = 63; i >= 0; i--)
            {
                intern = tempA[i] + tempB[i] + (intern >> 8);
                temp[i] = (byte)(intern & 0xFF);
            }
            return temp;
        }
        ...
        private byte[] HashS(byte[] state)
        {
            byte[] result = new byte[64];
            for (int i = 0; i < 64; i++)
                result[i] = SConsts[state[i]];
            return result;
        }

        private byte[] HashP(byte[] state)
        {
            byte[] result = new byte[64];
            for (int i = 0; i < 64; i++)
            {
                result[i] = state[TILDA[i]];
            }
            return result;
        }

        private byte[] HashL(byte[] state)
        {
            byte[] result = new byte[64];
            for (int i = 0; i < 8; i++)
            {
        ...
        public byte[] CalculateHash(byte[] message)
        {
            ...
            int inc = 0;
            while (messageLength >= 512)
            {
                inc++;
                byte[] tempMes = new byte[64];
                Array.Copy(message, message.Length - inc * 64, tempMes, 0, 64);
                h = G_n(N, h, tempMes);
                N = HashAdd512(N, N_512.Reverse().ToArray());
                Sigma = HashAdd512(Sigma, tempMes);
                messageLength -= 512;
            }
            ...
            N = HashAdd512(N, MesLen.Reverse().ToArray());
            Sigma = HashAdd512(Sigma, paddedMes);
            h = G_n(N_0, h, N);
            h = G_n(N_0, h, Sigma);
            if (outLen == 512)
                return h;
        ...
        {
            Stribog input256 = new Stribog(256);
            Stribog input512 = new Stribog(512);

            Console.WriteLine($"Хешируемое сообщение: ");
            byte[] message ={
            ...
            Console.WriteLine("323130393837363534333231303938373635343332313039383736353433323130393837363534333231303938373635343332313039383736353433323130");

            byte[] result256 = input256.CalculateHash(message);
            byte[] result512 = input512.CalculateHash(message);
            string hesh256 = BitConverter.ToString(result256);
            string hesh512 = BitConverter.ToString(result512);


            Console.WriteLine("Хеш 256:");
            Console.WriteLine(hesh256);
            Console.WriteLine("Хеш 512:");
            Console.WriteLine(hesh512);
        }
